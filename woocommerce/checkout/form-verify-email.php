...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email"
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( "Nous n'avons pas pu vérifier l'adresse e-mail que vous avez fournie. Veuillez réessayer.", 'woocommerce' ), 'error' );
	}
	?>
    <p>
        <?php
    ...
    <p class="form-row">
        <label for="email"><?php esc_html_e( 'Adresse email', 'woocommerce' ); ?>&nbsp;<span
    ...
    <p class="form-row">
        <button type="submit"
            ...
            name="verify" value="1">
            <?php esc_html_e( 'Vérifier', 'woocommerce' ); ?>
        </button>
    </p>
</form>